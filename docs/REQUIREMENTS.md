...
ZI/O 将笔记、阅读、观影、游戏、代码项目和日记组织到同一条时间轴中。它不是社交信息流，也不是多个媒体追踪器的拼接。
...
- 图书、电影、游戏、代码项目等作为可长期积累的 Item，与多条 Note 关联。
...
Item 是可长期存在的档案对象，例如一本书、一部电影、一个游戏或代码项目。不同类型统一存于 `items`，不为每种媒体建立独立业务表。
...
- `/books`、`/movies`、`/games`、`/code`、`/journal`：对应档案入口。
