...
<template>
  <section class="reference-index" aria-labelledby="reference-index-title">
    <div class="section-heading">
      <div class="eyebrow mono">{{ t('index.eyebrow') }}</div>
      <h1 id="reference-index-title">{{ t('index.title') }}</h1>
    </div>
    ...
    <template v-else>
      <div class="index-columns">
        <div>
          ...
          >
            <span>{{ category.is_root ? categoryLabel(category.code, category.name) : category.name }}</span>
            <span class="mono">{{ category.note_count }}</span>
          </RouterLink>
        </div>

        <div>
          <h2 class="subheading">{{ t('index.tags') }}</h2>
          <RouterLink v-for="tag in data.tags" :key="tag.id" class="index-row" :to="`/tag/${tag.slug}`">
            <span>#{{ tag.name }}</span>
      ...
      <h2 class="subheading">{{ t('index.publicItems') }}</h2>
